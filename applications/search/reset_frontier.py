...
from datamodel.search.Pamelac2Syr1_datamodel import Pamelac2Syr1ProjectionLink
...
@Deleter(Pamelac2Syr1ProjectionLink)
@Getter(Pamelac2Syr1ProjectionLink)
class DeleteFrontierFrame(IApplication):

    def __init__(self, frame):
        self.starttime = time()
        self.app_id = "Pamelac2Syr1"
        self.frame = frame
    ...
    def update(self):
        ...
        ls = self.frame.get(Pamelac2Syr1ProjectionLink)
        print "Found ", len(ls), " links to delete."
        for l in ls:
            self.frame.delete(Pamelac2Syr1ProjectionLink, l)
